...
The manifest's optional top-level `approval` section is exposed unchanged as `AgentControl.approval_config`, read from the fully merged manifest by the native core on every constructor. `HostSession` bounds its synchronous approval wait with that section's `timeout_seconds` when declared and with `DEFAULT_APPROVAL_TIMEOUT_SECONDS` otherwise; an explicit `approval_timeout_seconds=` argument wins over both.
