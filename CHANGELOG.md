...
### Changed
- **BREAKING: Python `HostSession` sends the snapshot contract's keys** — `model_request`, `model_response` and `output` instead of top-level `model`/`messages`/`tools`, `response` and `response.content`, so one manifest binds through `HostSession` and the framework adapters alike. See BREAKING_CHANGES.md for the path rewrites.
