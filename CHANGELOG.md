...
### Changed
- **BREAKING: Python `Decision.applies_effects` removed** — deprecated since the AGT D1 effects removal; use `applies_transform` or `permits`. The `DeprecationWarning` it emitted on access broke suites run with `-W error`.
